	...
	while (true) {
		...
		static if (isServer) {
			if (selectSet.isSet(server)) {
				try {
					auto client = new AuthClient(server.accept(), nextSid++);
					addToThread(client);
...
*/
private void addToThread(AuthClient client) {
	synchronized {
		import main : settings;
		if (clients.length >= settings.read!size_t("ClientsPerThread")) {
...
*/
private void removeSocket(AuthClient client) {
	try {
