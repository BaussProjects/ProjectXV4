module crypto.authcrypto;

/**
*	The auth crypto. (Essential the same as the game crypto.)
*/
class AuthCrypto {
private:
	/**
	* The first keys.
	*/
	ubyte[] keys1 = [
		
	];
	
	/**
	*	The second keys.
	*/
	ubyte[] keys2 = [
		
	];
	
	/**
	*	The in counter.
	*/
	ushort inCounter = 0;
	/**
	*	The out counter.
	*/
	ushort outCounter = 0;
public:
	/**
	*	Creates a new instance of AuthCrypto.
	*/
	this() {
	}
	
	/**
	*	Encrypts a buffer.
	*	Params:
	*		inBuffer =	The in buffer.
	*		outBuffer =	The out buffer.
	*		size =		The size of the buffers.
	*/
	void encrypt(ubyte[] inBuffer, ubyte[] outBuffer, int size) {
		synchronized {
			foreach (i; 0 .. size) {
				outBuffer[i] = cast(ubyte)(inBuffer[i] ^ 0xAB);
				outBuffer[i] = cast(ubyte)((outBuffer[i] << 4) | (outBuffer[i] >> 4));
				outBuffer[i] = cast(ubyte)(keys2[outCounter >> 8] ^ outBuffer[i]);
				outBuffer[i] = cast(ubyte)(keys1[outCounter & 0xFF] ^ outBuffer[i]);
			
				outCounter++;
			}
		}
	}
	
	/**
	*	Decrypts an encrypted buffer.
	*	Params:
	*		inBuffer =	The in buffer.
	*		outBuffer =	The out buffer.
	*		size =		The size of the buffers.
	*/
	void decrypt(ubyte[] inBuffer, ubyte[] outBuffer, int size) {
		synchronized {
			foreach (i; 0 .. size) {
				outBuffer[i] = cast(ubyte)(inBuffer[i] ^ 0xAB);
				outBuffer[i] = cast(ubyte)((outBuffer[i] << 4) | (outBuffer[i] >> 4));
				outBuffer[i] = cast(ubyte)(keys2[inCounter >> 8] ^ outBuffer[i]);
				outBuffer[i] = cast(ubyte)(keys1[inCounter & 0xFF] ^ outBuffer[i]);
			
				inCounter++;
			}
		}
	}
}