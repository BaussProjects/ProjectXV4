...
public:
	...
	*/
	this(Socket socket, size_t sid) {
		...
		m_equipments = new Equipments(this);
	}
	...
	*/
	...
	@property {
		...
		*	Gets the equipments.
		*/
		Equipments equipments() { return m_equipments; }
	...
	/**
	*	Updates the default stat points.
	*/
	void updateStatPoints() {
		if (m_reborns == 0 && level <= 120) {
			import core.kernel;
			import std.stdio : writeln;
			auto basejob = getBaseJob(cast(ubyte)m_job);
			auto stats = getKernelStats(basejob, m_level);
			strength = stats[0];
			agility = stats[1];
			vitality = stats[2];
			agility = stats[3];
	...
	*/
	void handleAsyncReceive() {
		...
		synchronized {
			recvBuf = m_crypto.decrypt(recvBuf);
		}
		
		if (m_isHead) {
			scope auto head = new DataPacket(recvBuf);
